// 到件扫描结果判定纯函数
// C1-2/C1-3: 从单一 boolean 改为多态判定，避免 toast 缺失误判 + 成功判定过宽
// Phase L-2: 新增反向数量匹配（2条成功）、部分成功独立检测、变体文案覆盖
 ...
 */
export type ArriveScanStatus = 'SUCCESS' | 'PARTIAL' | 'FAILED' | 'UNKNOWN_NEEDS_MANUAL_CHECK';

export interface ArriveScanOutcome {
  status: ArriveScanStatus;
  /** 成功条数，无法解析时为 null */
 ...
 *    - N=0 且 M>0 → FAILED
 *    - N>0 且 M>0 → PARTIAL
 *    - 此外，若文案含"部分成功/部分失败"关键词 → 强制 PARTIAL
 ...
 */
export function parseArriveScanResult(toastText: string, batchSize: number): ArriveScanOutcome {
  const text = (toastText ?? '').trim();

  // C1-2: toast 缺失 → UNKNOWN，不等于 FAILED
  if (!text) {
    return {
  ...
  if (rawSuccessCount !== null || rawFailCount !== null) {
    ...
    // Phase L-2: "部分成功/部分失败" 关键词 → 强制 PARTIAL（即使数量全为成功也要标 PARTIAL）
  ...
  // Phase L-2: "部分成功/部分失败" 纯文案（无数量）→ PARTIAL
  ...
  // 纯成功文案（无具体数量）
  if (/批量到件成功|到件成功|操作成功|全部成功|提交成功/.test(text)) {
    return { status: 'SUCCESS', successCount: batchSize, failCount: 0, message: text };
  }

  // 纯失败文案（无具体数量）
  if (/操作失败|到件失败|批量到件失败|全部失败|提交失败/.test(text)) {
